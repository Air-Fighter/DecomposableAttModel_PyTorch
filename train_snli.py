...
from model import DecomposableModel
...
train, dev, test = datasets.SNLI.splits(inputs, answers, root=args.snli_root)
...
for epoch in range(args.epochs):
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        train_acc = 100. * float(n_correct)/float(n_total)
        ...
        if iterations % args.dev_every == 0:
            ...
            print(log_template.format(time.time()-start,
                epoch, iterations, 1+batch_idx, len(train_iter),
                100. * (1+batch_idx) / len(train_iter), loss.data[0], ' '*8, float(n_correct)/float(n_total)*100, ' '*12))
